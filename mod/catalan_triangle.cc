...
mod_int catalan(int n) {
    combo.prepare_factorials(2 * n + 1);
    // return choose(2 * n, n) - choose(2 * n, n + 1);
    return combo._factorial[2 * n] * combo._inv_factorial[n] * combo._inv_factorial[n + 1];
}
 
// https://en.wikipedia.org/wiki/Catalan%27s_triangle
// The number of paths to get from point (0, 0) to point (n, k) without ever crossing (strictly) above the line y = x.
// Note that catalan(n) = catalan_triangle(n, n).
mod_int catalan_triangle(int64_t n, int64_t k) {
    if (k < 0 || k > n) return 0;
    combo.prepare_factorials(n + k + 1);
    // return choose(n + k, k) - choose(n + k, k - 1);
    return combo._factorial[n + k] * (n - k + 1) * combo._inv_factorial[k] * combo._inv_factorial[n + 1];
}
